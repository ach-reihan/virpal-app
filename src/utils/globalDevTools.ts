...
function initializeDevTools(): void {
  ...
  const devTools: VirpalDevTools = {
    ...
    async getAccessToken(): Promise<string | null> {
      try {
        ...
        console.log('🔑 Access Token:', token ? 'Retrieved successfully' : 'No token available');
        return token;
      } catch (error) {
    ...
    async loginAndGetToken(): Promise<string | null> {
      try {
        ...
        // Get token
        const token = await authService.getAccessToken();
        console.log('✅ Login successful! Token:', token ? 'Retrieved' : 'Failed to get token');
        return token;
      } catch (error) {
    ...
    async testApiEndpoints(): Promise<void> {
      ...
      for (const endpoint of endpoints) {
        try {
          ...
          if (endpoint.method === 'POST' && endpoint.path === '/api/chat-completion') {
  ...
  console.log('  await window.virpalDev.getAccessToken()           // Get current token');
  console.log('  await window.virpalDev.loginAndGetToken()         // Login and get token');
  console.log('  await window.virpalDev.copyTokenToClipboard()     // Copy token to clipboard');
  console.log('  await window.virpalDev.testApiEndpoints()         // Test all API endpoints');
  console.log('  window.virpalDev.authService.getCurrentUser()     // Get current user info');
}
