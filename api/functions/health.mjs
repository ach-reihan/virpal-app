...
async function checkServiceConnectivity(context) {
    ...
    if (process.env['AZURE_TENANT_ID']) {
        serviceChecks.push(checkService('tenant', async () => {
            return {
                status: 'configured',
    ...
    if (process.env['COSMOS_DB_ENDPOINT']) {
        serviceChecks.push(checkService('cosmos', async () => {
    ...
    if (process.env['AZURE_OPENAI_ENDPOINT']) {
        serviceChecks.push(checkService('openAI', async () => {
    ...
    results.forEach((result) => {
        if (result.status === 'fulfilled') {
            ...
            totalCount++;
            if (serviceResult.status === 'configured' ||
 ...
 */
async function checkService(name, checkFn, timeoutMs) {
    const startTime = Date.now();
    try {
