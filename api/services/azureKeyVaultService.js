...
class AzureKeyVaultService {
    ...
    async performInitialization() {
        try {
            ...
            if (!this.keyVaultUrl.startsWith('https://') ||
                ...
                logger.error('Invalid Key Vault URL format. Expected: https://<vault-name>.vault.azure.net/');
                return;
            }
    ...
    async getSecret(secretName) {
        try {
            ...
            const secret = await this.retryOperation(() => this.secretClient.getSecret(sanitizedSecretName), 3, // max retries
            1000 // base delay ms
            );
            if (secret.value) {
            ...
            if (error.code === 'SecretNotFound') {
                ...
                logger.error('Access denied to Key Vault. Check Managed Identity permissions.');
            }
     ...
     */
    async retryOperation(operation, maxRetries, baseDelayMs) {
        let lastError;
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                logger.debug(`Retrying Key Vault operation in ${delay}ms (attempt ${attempt}/${maxRetries})`);
    ...
    async healthCheck() {
        ...
        try {
            ...
            return {
                status: 'degraded',
                details: `Key Vault connectivity issue: ${error.code || 'Unknown error'}`,
                responseTime: Date.now() - startTime,
            };
        }
    }
...
const isAzureFunctionsEnvironment = () => {
    try {
...
if (isNodeJSEnvironment()) {
    if (isAzureFunctionsEnvironment()) {
        ...
        logger.info('Initializing Key Vault service for Azure Functions environment');
        keyVaultService = new AzureKeyVaultService();
    }
    else {
        // Running in Node.js but not Azure Functions (development build)
        logger.debug('Initializing Key Vault service for Node.js development environment');
        keyVaultService = new AzureKeyVaultService();
    }
}
else {
